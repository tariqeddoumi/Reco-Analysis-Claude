      ...
      {STEPS.map((step, i) => {
        ...
        return (
          <React.Fragment key={step.id}>
            ...
            {i < STEPS.length - 1 && (
              <div className={cn("flex-1 h-0.5 mt-[-16px] mb-auto transition-colors", step.id < currentStep ? "bg-primary" : "bg-border")} />
            )}
          </React.Fragment>
        );
      })}
...
function ImpactSlider({ label, value, onChange }: { label: string; value: number | undefined; onChange: (v: number) => void }) {
...
export default function EditRecommendationPage() {
  ...
  React.useEffect(() => {
    ...
    fetch(`/api/recommendations/${id}`)
      ...
      .then((data) => {
        reset({
          missionId: data.missionId ?? data.mission?.id ?? "",
          ...
          rootCauseTypeId: data.rootCauseTypeId ?? "",
          riskTypeId: data.riskTypeId ?? "",
          severityId: data.severityId ?? "",
          probabilityId: data.probabilityId ?? "",
          priorityId: data.priorityId ?? "",
          ownerId: data.ownerId ?? "",
          operationalResponsibleId: data.operationalResponsibleId ?? "",
          confidentialityLevelId: data.confidentialityLevelId ?? "",
          issuedAt: toDateInput(data.issuedAt),
          initialDueDate: toDateInput(data.initialDueDate),
          findingDescription: data.findingDescription ?? "",
          recommendation: data.recommendation ?? "",
          rootCause: data.rootCause ?? "",
          potentialConsequence: data.potentialConsequence ?? "",
          entityComment: data.entityComment ?? "",
          reportReference: data.reportReference ?? "",
          pageReference: data.pageReference ?? "",
          isRegulator: data.isRegulator ?? false,
          isRecurrent: data.isRecurrent ?? false,
          regulatoryImpact: data.regulatoryImpact ?? undefined,
          reputationalImpact: data.reputationalImpact ?? undefined,
          operationalImpact: data.operationalImpact ?? undefined,
          clientImpact: data.clientImpact ?? undefined,
          siImpact: data.siImpact ?? undefined,
          legalImpact: data.legalImpact ?? undefined,
          complianceImpact: data.complianceImpact ?? undefined,
    ...
    <AppLayout>
      <div className="space-y-6 max-w-3xl mx-auto">
        <nav className="flex items-center gap-1.5 text-sm text-muted-foreground">
          ...
          <ChevronRight className="h-3.5 w-3.5" />
          <button onClick={() => router.back()} className="hover:text-foreground transition-colors font-mono text-xs">
            Détail
          </button>
          <ChevronRight className="h-3.5 w-3.5" />
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Modifier la recommandation</h1>
            <p className="text-sm text-muted-foreground mt-0.5">Mettre à jour les informations en 3 étapes</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <FormField label="Source" required error={errors.sourceId?.message}>
                  <Controller name="sourceId" control={control} render={({ field }) => (
                    <Select value={field.value ?? ""} onValueChange={field.onChange}>
                      <SelectTrigger className={cn(errors.sourceId && "border-destructive")}><SelectValue placeholder="Source" /></SelectTrigger>
                ...
                <FormField label="Entité" required error={errors.entityId?.message}>
                  <Controller name="entityId" control={control} render={({ field }) => (
                    <Select value={field.value ?? ""} onValueChange={field.onChange}>
                      <SelectTrigger className={cn(errors.entityId && "border-destructive")}><SelectValue placeholder="Entité" /></SelectTrigger>
                ...
                <FormField label="Date d'émission" required error={errors.issuedAt?.message}>
                  <Input {...register("issuedAt")} type="date" className={cn(errors.issuedAt && "border-destructive")} />
                </FormField>
                <FormField label="Type de recommandation">
                  <Controller name="recommendationTypeId" control={control} render={({ field }) => (
                    <Select value={field.value ?? ""} onValueChange={field.onChange}>
                      <SelectTrigger><SelectValue placeholder="Type (optionnel)" /></SelectTrigger>
                      <SelectContent>{referentials.recommendationTypes?.map((t) => <SelectItem key={t.id} value={t.id}>{t.label}</SelectItem>)}</SelectContent>
                    </Select>
                  )} />
                </FormField>
                <FormField label="Direction">
                  <Controller name="directionId" control={control} render={({ field }) => (
                    <Select value={field.value ?? ""} onValueChange={field.onChange}>
                      <SelectTrigger><SelectValue placeholder="Direction (optionnel)" /></SelectTrigger>
                      <SelectContent>{referentials.directions?.map((d) => <SelectItem key={d.id} value={d.id}>{d.label}</SelectItem>)}</SelectContent>
                    </Select>
                  )} />
                </FormField>
                <FormField label="Processus">
                  <Controller name="processId" control={control} render={({ field }) => (
                    <Select value={field.value ?? ""} onValueChange={field.onChange}>
                      <SelectTrigger><SelectValue placeholder="Processus (optionnel)" /></SelectTrigger>
                      <SelectContent>{referentials.processes?.map((p) => <SelectItem key={p.id} value={p.id}>{p.label}</SelectItem>)}</SelectContent>
                    </Select>
                  )} />
                </FormField>
                ...
                <FormField label="Réf. rapport"><Input {...register("reportReference")} placeholder="Référence du rapport" /></FormField>
                <FormField label="Réf. page"><Input {...register("pageReference")} placeholder="N° de page" /></FormField>
              </CardContent>
            </Card>
          )}
            ...
            <Card>
              ...
              <CardContent className="space-y-5">
                <FormField label="Description du constat" required error={errors.findingDescription?.message}>
                  <Textarea {...register("findingDescription")} placeholder="Décrire le constat observé..." rows={5} className={cn(errors.findingDescription && "border-destructive")} />
                </FormField>
                <FormField label="Cause racine">
                  <Controller name="rootCauseTypeId" control={control} render={({ field }) => (
                    <Select value={field.value ?? ""} onValueChange={field.onChange}>
                      <SelectTrigger><SelectValue placeholder="Type de cause racine (optionnel)" /></SelectTrigger>
                      <SelectContent>{referentials.rootCauseTypes?.map((r) => <SelectItem key={r.id} value={r.id}>{r.label}</SelectItem>)}</SelectContent>
                    </Select>
                  )} />
                </FormField>
                <FormField label="Description cause racine">
                  <Textarea {...register("rootCause")} placeholder="Analyser la cause fondamentale..." rows={3} />
                </FormField>
                <Separator />
                <FormField label="Conséquence potentielle">
                  <Textarea {...register("potentialConsequence")} placeholder="Décrire les conséquences potentielles..." rows={3} />
                </FormField>
                <Separator />
                <FormField label="Recommandation" required error={errors.recommendation?.message}>
                  <Textarea {...register("recommendation")} placeholder="Formuler la recommandation de manière claire et actionnable..." rows={5} className={cn(errors.recommendation && "border-destructive")} />
                </FormField>
                <FormField label="Commentaire entité">
                  <Textarea {...register("entityComment")} placeholder="Commentaire de l'entité concernée..." rows={3} />
                </FormField>
              </CardContent>
            </Card>
          )}
              ...
              <Card>
                ...
                <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <FormField label="Sévérité">
                    <Controller name="severityId" control={control} render={({ field }) => (
                      <Select value={field.value ?? ""} onValueChange={field.onChange}>
                        <SelectTrigger><SelectValue placeholder="Niveau de sévérité" /></SelectTrigger>
                        <SelectContent>{referentials.severities?.map((s) => <SelectItem key={s.id} value={s.id}>{s.label}</SelectItem>)}</SelectContent>
                      </Select>
                    )} />
                  </FormField>
                  <FormField label="Probabilité">
                    <Controller name="probabilityId" control={control} render={({ field }) => (
                      <Select value={field.value ?? ""} onValueChange={field.onChange}>
                        <SelectTrigger><SelectValue placeholder="Niveau de probabilité" /></SelectTrigger>
                        <SelectContent>{referentials.probabilities?.map((p) => <SelectItem key={p.id} value={p.id}>{p.label}</SelectItem>)}</SelectContent>
                      </Select>
                    )} />
                  </FormField>
                  <FormField label="Priorité">
                    <Controller name="priorityId" control={control} render={({ field }) => (
                      <Select value={field.value ?? ""} onValueChange={field.onChange}>
                        ...
                        <SelectContent>{referentials.priorities?.map((p) => <SelectItem key={p.id} value={p.id}>{p.label}</SelectItem>)}</SelectContent>
                      </Select>
                    )} />
                  </FormField>
                  <FormField label="Type de risque">
                    <Controller name="riskTypeId" control={control} render={({ field }) => (
                      <Select value={field.value ?? ""} onValueChange={field.onChange}>
                        <SelectTrigger><SelectValue placeholder="Type de risque" /></SelectTrigger>
                        <SelectContent>{referentials.riskTypes?.map((r) => <SelectItem key={r.id} value={r.id}>{r.label}</SelectItem>)}</SelectContent>
                      </Select>
                    )} />
                  </FormField>
                  ...
                  <FormField label="Resp. opérationnel">
                    <Controller name="operationalResponsibleId" control={control} render={({ field }) => (
                      <Select value={field.value ?? ""} onValueChange={field.onChange}>
                        <SelectTrigger><SelectValue placeholder="Responsable opérationnel" /></SelectTrigger>
                        <SelectContent>{referentials.users?.map((u) => <SelectItem key={u.id} value={u.id}>{u.firstName} {u.lastName}</SelectItem>)}</SelectContent>
                      </Select>
                    )} />
                  </FormField>
                  <FormField label="Échéance initiale">
                  ...
                  <FormField label="Confidentialité">
                    <Controller name="confidentialityLevelId" control={control} render={({ field }) => (
                      <Select value={field.value ?? ""} onValueChange={field.onChange}>
                        <SelectTrigger><SelectValue placeholder="Niveau de confidentialité" /></SelectTrigger>
                  ...
                  <div className="flex items-center justify-between rounded-lg border border-border p-4">
                    ...
                    <Controller name="isRegulator" control={control} render={({ field }) => (
                      <Switch id="isRegulator" checked={field.value} onCheckedChange={field.onChange} />
                    )} />
                  </div>
                  <div className="flex items-center justify-between rounded-lg border border-border p-4">
                    ...
                    <Controller name="isRecurrent" control={control} render={({ field }) => (
                      <Switch id="isRecurrent" checked={field.value} onCheckedChange={field.onChange} />
                    )} />
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="space-y-3">
                  {([
                    ...
                    <Controller
                      ...
                      control={control}
                      render={({ field: f }) => (
                        <ImpactSlider label={label} value={f.value as number | undefined} onChange={(v) => f.onChange(v)} />
                      )}
                    />
                  ))}
                  <Separator className="my-2" />
                  <FormField label="Impact financier (€)">
                    <Input {...register("financialImpact", { valueAsNumber: true })} type="number" min={0} placeholder="Montant estimé en euros" />
                  </FormField>
                </CardContent>
              </Card>
            </div>
          ...
          <div className="flex items-center justify-between pt-4 pb-8">
            <Button type="button" variant="outline" onClick={step === 1 ? () => router.back() : handleBack} className="gap-2">
              <ChevronLeft className="h-4 w-4" />
              {step === 1 ? "Annuler" : "Précédent"}
            </Button>
            <Badge variant="secondary" className="text-xs">Étape {step} / {STEPS.length}</Badge>
            {step < 3 ? (
